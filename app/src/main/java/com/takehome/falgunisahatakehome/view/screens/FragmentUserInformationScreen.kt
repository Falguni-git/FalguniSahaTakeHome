...
class FragmentUserInformationScreen: Fragment(),ListAdapter.ClickListener {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        val userInformation =LayoutInflater.from(container!!.context).inflate(R.layout.layout_fragment_user_information_screen,container,false)
    ...
    private fun subscribe() {
        ...
        activity?.let {
            mainViewModel.isLoading.observe(it) { isLoading ->
        ...
        activity?.let {
            ...
            mainViewModel.isError.observe(it) { isError ->
                if (isError) errorDialog.show() else errorDialog.cancel()
            }
        }

        activity?.let {
            mainViewModel.userData.observe(it) { data ->
                userName.text = data.name
                Glide.with(this).load(data.avatarUrl).into(userImage)
            }
        }
        activity?.let {
            mainViewModel.userRepo.observe(it) { data ->
